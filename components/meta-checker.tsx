...
import { ArrowRight, Copy, Check, ExternalLink, X, RefreshCw, AlertCircle } from "lucide-react"
...
export default function MetaChecker() {
  ...
  const [suggestedUrls] = useState<string[]>(["vercel.com", "github.com", "nextjs.org", "tailwindcss.com"])
  ...
  const fetchMetadata = async (urlToFetch: string) => {
    ...
    try {
      ...
      const response = await fetch(`/api/metadata?url=${encodeURIComponent(formattedUrl)}&full=true`).catch(() => null)
        ...
        <div>
            ...
            <div className="space-y-8">
              <div className="bg-white dark:bg-[#111111] rounded-lg border border-gray-100 dark:border-[#222222] overflow-hidden">
                <div className="px-4 py-3 bg-gray-50 dark:bg-[#181818] border-b border-gray-100 dark:border-[#222222]">
                  <h3 className="text-sm font-medium dark:text-[#E0E0E0]">Essential Metadata</h3>
                </div>
                <div className="p-4 space-y-1">
                  {(metadata.favicon || url) && (
                    <div className="flex items-center gap-3 py-2">
                      <div className="text-xs font-medium text-gray-500 dark:text-gray-400 w-32">Favicon</div>
                      <a
                        href={metadata.favicon || `https://${url.replace(/^https?:\/\//, "")}/favicon.ico`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-block"
                      >
                        <img
                          src={metadata.favicon || `https://${url.replace(/^https?:\/\//, "")}/favicon.ico`}
                          alt="Favicon"
                            ...
                            if (url.includes("//")) {
                              ...
                              imgElement.onerror = () => {
                                ...
                                if (parent) {
                                  ...
                                  errorMsg.className = "text-gray-500 dark:text-gray-400 text-sm font-mono";
              ...
              <div className="bg-white dark:bg-[#111111] rounded-lg border border-gray-100 dark:border-[#222222] overflow-hidden">
                <div className="px-4 py-3 bg-gray-50 dark:bg-[#181818] border-b border-gray-100 dark:border-[#222222]">
                  <h3 className="text-sm font-medium dark:text-[#E0E0E0]">Technical Details</h3>
                </div>
                <div className="p-4 space-y-1">
              ...
              <div className="bg-white dark:bg-[#111111] rounded-lg border border-gray-100 dark:border-[#222222] overflow-hidden">
                <div className="px-4 py-3 bg-gray-50 dark:bg-[#181818] border-b border-gray-100 dark:border-[#222222]">
                  <h3 className="text-sm font-medium dark:text-[#E0E0E0]">Mobile & App</h3>
                </div>
                <div className="p-4 space-y-1">
                  <MetadataItem label="Mobile App" value={metadata.mobileApp} />
                  <MetadataItem label="Mobile App URL" value={metadata.mobileAppUrl} />
                  <MetadataItem label="Apple iTunes App" value={metadata.appleItunesApp} />
                  <MetadataItem label="Apple Mobile Web App Capable" value={metadata.appleMobileWebAppCapable} />
                  <MetadataItem label="Apple Mobile Web App Title" value={metadata.appleMobileWebAppTitle} />
                  <MetadataItem label="Format Detection" value={metadata.formatDetection} />
                </div>
              </div>
          ...
          {activeTab === "opengraph" && (
            <div className="space-y-8">
              <div className="bg-white dark:bg-[#111111] rounded-lg border border-gray-100 dark:border-[#222222] overflow-hidden">
                <div className="px-4 py-3 bg-gray-50 dark:bg-[#181818] border-b border-gray-100 dark:border-[#222222]">
                  <h3 className="text-sm font-medium dark:text-[#E0E0E0]">Open Graph Basic</h3>
                </div>
                <div className="p-4 space-y-1">
                  <MetadataItem label="OG Title" value={metadata.ogTitle} characterCount />
                  <MetadataItem label="OG Description" value={metadata.ogDescription} characterCount />
                  <MetadataItem label="OG Image" value={metadata.ogImage} isImage />
                </div>
              </div>

              <div className="bg-white dark:bg-[#111111] rounded-lg border border-gray-100 dark:border-[#222222] overflow-hidden">
                <div className="px-4 py-3 bg-gray-50 dark:bg-[#181818] border-b border-gray-100 dark:border-[#222222]">
                  <h3 className="text-sm font-medium dark:text-[#E0E0E0]">Open Graph Additional</h3>
                </div>
                <div className="p-4 space-y-1">
                  <MetadataItem label="OG Type" value={metadata.ogType} />
                  <MetadataItem label="OG URL" value={metadata.ogUrl} />
                  <MetadataItem label="OG Site Name" value={metadata.ogSiteName} />
                ...
                <div className="p-4 bg-gray-50 dark:bg-[#181818] rounded-lg border border-gray-200 dark:border-[#222222] shadow-sm dark:shadow-inner">
                  ...
                  <div className="space-y-2">
                    ...
                    {!metadata.ogType ? (
                      <p className="text-xs text-amber-500 dark:text-amber-400 flex items-center">
                        <span className="inline-block w-2 h-2 rounded-full bg-amber-500 mr-2"></span>
                        Missing og:type. Consider adding a type (e.g., website, article).
                      </p>
                    ) : (
                      <p className="text-xs text-green-500 dark:text-green-400 flex items-center">
                        <span className="inline-block w-2 h-2 rounded-full bg-green-500 mr-2"></span>
            ...
            <div className="space-y-8">
              <div className="bg-white dark:bg-[#111111] rounded-lg border border-gray-100 dark:border-[#222222] overflow-hidden">
                <div className="px-4 py-3 bg-gray-50 dark:bg-[#181818] border-b border-gray-800/60">
                  <h3 className="text-sm font-medium dark:text-[#E0E0E0]">X Card</h3>
                </div>
                <div className="p-4 space-y-1">
                  <MetadataItem label="Card Type" value={metadata.twitterCard} />
                  <MetadataItem label="Title" value={metadata.twitterTitle} characterCount />
                  <MetadataItem label="Description" value={metadata.twitterDescription} characterCount />
                  <MetadataItem label="Image" value={metadata.twitterImage} isImage />
                </div>
              </div>
          ...
          {activeTab === "images" && (
            <div className="space-y-6">
              {metadata.ogImage && <ImagePreview title="Open Graph Image" url={metadata.ogImage} />}
              {metadata.twitterImage && <ImagePreview title="X Image" url={metadata.twitterImage} />}
              {metadata.favicon && <ImagePreview title="Favicon" url={metadata.favicon} />}
              {!metadata.ogImage && !metadata.twitterImage && !metadata.favicon && (
                <div className="p-4 bg-gray-50 dark:bg-[#181818] rounded-md border border-gray-200 dark:border-[#222222] text-center">
                  <p className="text-gray-500 dark:text-gray-400 font-mono text-sm">No images found in metadata</p>
                </div>
              )}
              ...
              {showSuggestions && (
                <div className="mt-6 p-3 bg-gray-50 dark:bg-[#181818] rounded-md border border-gray-200 dark:border-[#222222] shadow-sm dark:shadow-inner">
                  <h3 className="text-sm font-medium mb-2 text-black dark:text-[#E0E0E0]">Suggestions</h3>
                  {!metadata.ogImage && !metadata.twitterImage ? (
                    <p className="text-xs text-red-500 dark:text-red-400">
                      No social media images found. Add og:image and twitter:image.
                    </p>
                  ) : (
                    <p className="text-xs text-green-500 dark:text-green-400">Social media images are present.</p>
                  )}

                  {!metadata.favicon ? (
                    <p className="text-xs text-amber-500 dark:text-amber-400">
                      No favicon detected. Add a favicon for better brand recognition.
                    </p>
                  ) : (
                    <p className="text-xs text-green-500 dark:text-green-400">Favicon is present.</p>
                  )}
                </div>
              )}
            </div>
          ...
          {activeTab === "raw" && (
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                >
                  {copied ? (
                    ...
                    <>
                      <Copy size={14} className="group-hover:text-[#444444] transition-colors" />
              ...
              <div className="text-xs text-gray-500 dark:text-gray-400">
                <span className="font-medium">Tip:</span> Use the copy button to export this data for further analysis.
              </div>
            </div>
          )}
          ...
          {activeTab === "other" && (
            <div>
              ...
              <div className="mt-4">
                <PlatformPreview platform={activePlatform} metadata={metadata} url={url} />
                ...
                {showSuggestions && (
                  <div className="mt-6 p-3 bg-gray-50 dark:bg-[#181818] rounded-md border border-gray-200 dark:border-[#222222] shadow-sm dark:shadow-inner">
                    ...
                    {activePlatform === "twitter" && !metadata.twitterCard ? (
                      <p className="text-xs text-red-500 dark:text-red-400">
                        Missing twitter:card. Add a card type for better display.
                      </p>
                    ) : activePlatform === "facebook" && !metadata.ogImage ? (
                      <p className="text-xs text-red-500 dark:text-red-400">
                        Missing og:image. Facebook requires an image for better sharing.
                      </p>
                    ) : activePlatform === "linkedin" && !metadata.ogTitle ? (
                      <p className="text-xs text-red-500 dark:text-red-400">
                        Missing og:title. LinkedIn requires a title for better sharing.
                      </p>
                    ) : (
